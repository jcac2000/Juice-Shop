...
By default, HTML forms try to send data via the URL (GET request) and refresh the page. This behavior is modified using $\color{red}{e.preventDefault()}$ to keep the credentials out of the browser history and to run the security validation process. Both email and password information are obtained using document.getElementById().value function from input fields.
